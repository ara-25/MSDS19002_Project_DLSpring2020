...
class SCAEMNIST(LightningModule):
    ...
    def prepare_data(self):
        ...
        mnist_train = ImageFolder("/content/cropped_months/train", transform=self.make_transforms())
        ...
        mnist_test = ImageFolder("/content/cropped_months/test", transform=self.make_transforms())
    ...
    def validation_step(self, batch, batch_idx):
        ...
        loss, loss_info = self.scae.loss(res,
                                         ...
                                         label=label)
        accuracy = self.scae.calculate_accuracy(res, label)
        accuracy_cl = self.scae.calculate_accuracy_dense(res, label)

        out = {'val_loss': loss, 'accuracy': accuracy, 'accuracy_dense': accuracy_cl}
    ...
    def validation_epoch_end(self, outputs):
        ...
        avg_acc_dense = torch.stack([x['accuracy_dense'] for x in outputs]).mean()
        log = {'val_loss': avg_loss, 'val_accuracy': avg_acc, 'val_acc_dense': avg_acc_dense}
    ...
    def test_step(self, batch, batch_idx):
        ...
        loss = self.scae.loss(res,
                              ...
                              label=label)
        accuracy = self.scae.calculate_accuracy(res, label)
        accuracy_dense = self.scae.calculate_accuracy_dense(res, label)

        return {'test_loss': loss, 'accuracy': accuracy, 'accuracy_dense': accuracy_dense}

    def test_epoch_end(self, outputs):
        avg_loss = torch.stack([x['test_loss'] for x in outputs]).mean()
        avg_acc = torch.stack([x['accuracy'] for x in outputs]).mean()
        avg_acc_dense = torch.stack([x['accuracy_dense'] for x in outputs]).mean()
        log = {'test_loss': avg_loss, 'test_accuracy': avg_acc, 'test_accuracy_dense': avg_acc_dense}
