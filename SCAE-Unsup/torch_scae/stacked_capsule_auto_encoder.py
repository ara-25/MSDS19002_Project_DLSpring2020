...
class SCAE(nn.Module):
    ...
    def __init__(
        ...
        if n_classes:
            self.prior_mlp = nn.Linear(6120, obj_encoder.n_outputs)
        ...
        #     nn.Linear(obj_encoder.dim_out*obj_encoder.n_outputs, 2048),
        #     nn.ReLU(),
        #     nn.Linear(2048, 256),
        #     nn.ReLU(),
    ...
    def forward(self, image):
        ...
        caps_lt_weight = torch.sigmoid(self.prior_mlp(image.view(image.shape[0], -1)))
        ...
        del target_pose, target_presence
        ...
        res.capsule_probs = caps_lt_weight
        # res.obj_encoding = obj_encoding
    ...
    # def calculate_accuracy_dense(self, res, label: torch.Tensor):
        # pred = res.pred_out
        # pred_a = torch.argmax(pred, axis=1)
        # acc = (pred_a == label).float().mean()
